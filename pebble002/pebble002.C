...
void GenerateSineValue(TH1D *hist, const double amp, const double freq, const double phase, const double offset) {
    ...
    const int nPoints = 1e5; // Number of points to generate
...
void fcn(int &npar, double *gin, double &ff, double *par, int iflag) {
    ...
    for (int ii = 1; ii <= nBins; ii++) {
        ...
        double fitValue = par[0] * sin(par[1] * xx + par[2]) + par[3];
...
void PlotSineFit(const int iteration, TF1 *fitFunc, const double *params, const double *errors, TList *list) {
    if (!gCanvas) {
...
void IterativeFit(TF1* fitFunc, double* params, double* errors, const int maxIterations, const double convergenceThreshold, TList *list) {
    ...
    for (int ii = 0; ii < maxIterations; ii++) {
        ...
        PlotSineFit(ii + 1, fitFunc, params, errors, list);
...
void FitWithROOT(TF1* fitFunc, double* params, double* errors) {
    gHist->Fit(fitFunc, "Q");
...
int main() {
    ...
    IterativeFit(fitFunc, params, errors, 10, 1e-6, plotList);
